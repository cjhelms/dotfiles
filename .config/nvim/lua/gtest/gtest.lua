...
function M.get_gtest_under_cursor()
...
function M.run_gtest(opts)
  local old_dir = vim.fn.getcwd()
  local temp_dir = opts.fargs[2]
  if temp_dir == nil then temp_dir = vim.g.gtest_temp_dir end
...
vim.api.nvim_create_user_command("GTestRun", M.run_gtest, {
  nargs = "+",
})
